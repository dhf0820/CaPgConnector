...
import (
	...
	"github.com/dhf0820/caDbConnector/src"
...
func CaToFhirDoc(ca m.DocumentSummary, patId string, mrn string ) (*fhir.Document, error) {
	//config := m.ActiveConfig()
	fd := fhir.Document{}
	fd.ID = primitive.NewObjectID().Hex()
	fd.FullURL = ""
	fd.EffectiveDateTime = ca.ReptDatetime
	fd.Meta.VersionID = "1"
	fd.Meta.LastUpdated = ca.ReptDatetime
	
	
	pat, err := src.GetPatientByMRN((mrn))
	if err != nil {
		err := fmt.Errorf("Patient:%s not found: %s", mrn, err.Error())
		log.Error(err)
		return nil, err
	}
	fd.Subject.Display = pat.Name
	fd.Subject.Reference = fmt.Sprintf("Patient/%s", patId)
	return nil, nil
