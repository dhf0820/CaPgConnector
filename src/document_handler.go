...
func WriteDocumentSummaryResponse(w http.ResponseWriter, resp *DocumentSummaryResponse) error {
...
func WriteDocumentClinicalDocumentResponse(w http.ResponseWriter, resp *ClinicalDocumentResponse) error {
	w.Header().Set("Content-Type", "application/json")
	w.WriteHeader(resp.StatusCode)
	err := json.NewEncoder(w).Encode(resp)
	if err != nil {
...
//QueryDocuments: Finds CA document summaries matching the provided params
func QueryDocuments(w http.ResponseWriter, r *http.Request) {
	...
	if r.Header.Get("AUTHORIZATION") == "" {
		...
		WriteDocumentSummaryResponse(w, &resp)
	}
	...
	if err != nil {
		log.Errorf("SetupDocumentFilter:161 -- failed: %s", err.Error())
		resp := DocumentSummaryResponse{}
		resp.StatusCode = 400
	...
	fmt.Printf("QueryDocuments:168 -- %s\n", spew.Sdump(docFilter))
